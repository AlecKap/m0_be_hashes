...
# states.each {|s, a| puts "#{s.capitalize} can be abreviated to #{a}."}
...
# birthdays.each {|name, bday| puts"Hello, my name is #{name}, and my birthday is on #{bday}."}
...
login_statuses.each {|n, lis|
  if lis == true
    puts "#{n} is currently logged in."
  end
}
